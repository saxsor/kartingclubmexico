...
const createSchema = z.object({
  ...
  diplomaNameX: z.number().min(0).max(1).optional(),
  diplomaNameY: z.number().min(0).max(1).optional(),
  diplomaNameWidth: z.number().min(0.01).max(1).optional(),
  diplomaNameHeight: z.number().min(0.01).max(1).optional(),
  diplomaFontSize: z.number().int().min(8).max(128).optional(),
  diplomaTextColor: z.string().regex(/^#[0-9A-Fa-f]{6}$/).optional(),
  diplomaTextAlign: z.enum(['left', 'center', 'right']).optional(),
